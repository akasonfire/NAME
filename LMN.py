...
def get_id():
  ...
  if id not in ids:
    ...
    get_id()
os.system('clear')
print(Colorate.Horizontal(Colors.white_to_blue, (f'\n</> ID SAVE IN - /sdcard/LMNx9_INSTA_HITS.txt\n')))
print(Colorate.Horizontal(Colors.red_to_green, (46*'•')))
def rest(user):
  try:
    headers = {
...
[*] rest : {rest(username)}
    '''
    print(Colorate.Horizontal(Colors.blue_to_green, (f'{tlg}')))
    with open('/sdcard/LMNx9_INSTA_HITS.txt','a') as ff:
      ff.write(f'{tlg}\n')
  except:
    tlg = f'''
|•••••••••••••••DARK-LMNx9•••••••••••••••|
...
with Live(console=console) as live:
  os.system('xdg-open https://t.me/Akasonfire11')
  while True:
    emoji=random.choice(['😆','😈','😐','😘','😻','😍','😹','😂','😭','🔥','⚡','🎁','🐸','🎭','👌','🔰','😱','🍁','🌼','🍌','🍑','🎃','🎲','🎯','🌍','🕜','🌟','🔔','📌','💊','🗿','🚬','📵','✅','❌','🔴','🔵','🏁','🍼','🍭','☕','🍷','🐼','🐷','🐱','🐽','🐯','🐶','😎','💩','😿','💝','👻','👽','💀','👊','✊','✋','👀','🐠','🐍','🐲','🌴','🌾','🌹','🌳','🍂','🍆','🌽','🍓','🍅','🍄','🍇','🍉','🍏','🎀','🏀','✨','🃏','🚥','⏰','💧','😁','😜','😶','😤','😇','💋','🙀','💚','💛','💙','💜','🌺'])
    tt=(f"\n\r[yellow][{emoji}] [[green]LMNx9-HitS][red] •[green] [{hits}][yellow] | [red][[green]Searching[red]][red] > [green][[red]{bads_instgram}[green]/[yellow]{bads_email}[/yellow][green]][/green]")
    live.update(tt)
    id=get_id()
    csrftoken = md5(str(time()).encode()).hexdigest()
